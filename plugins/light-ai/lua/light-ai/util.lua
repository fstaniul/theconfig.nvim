...
function M.exit_visual() vim.api.nvim_feedkeys(vim.api.nvim_replace_termcodes('<Esc>', true, false, true), 'x', false) end
...
---@param start_opts? vim.api.keyset.set_extmark
---@param end_opts? vim.api.keyset.set_extmark
---@return integer ns  The namespace id (unique per run_id).
---@return integer mark_start
---@return integer mark_end
function M.set_selection_marks(bufnr, run_id, start_line, start_col, end_line, end_col, start_opts, end_opts)
  start_opts = start_opts or {}
  end_opts = end_opts or {}
  local ns = vim.api.nvim_create_namespace('light-ai-' .. run_id)
  local ms = vim.api.nvim_buf_set_extmark(bufnr, ns, start_line - 1, start_col - 1, start_opts)
  local me = vim.api.nvim_buf_set_extmark(bufnr, ns, end_line - 1, end_col - 1, end_opts)
  return ns, ms, me
end
